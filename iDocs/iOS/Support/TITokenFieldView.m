...
- (id)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame titleText:@"To:" titleColor:nil titleFont:nil];
}
- (id)initWithFrame:(CGRect)frame titleText:(NSString *)newTitleText titleColor:(UIColor *)newTitleColor titleFont:(UIFont *)newTitleFont{
    ...
    if ((self = [super initWithFrame:frame])){
		...
		tokenField = [[TITokenField alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, kTokenFieldHeight) titleText:newTitleText titleColor:newTitleColor titleFont:newTitleFont];
...
//		[tokenField setBackgroundColor:[UIColor whiteColor]];
		[tokenField setBackgroundColor:[UIColor clearColor]];
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField {
	...
	if ([delegate respondsToSelector:@selector(tokenFieldShouldEndEditing:)]){
		[delegate tokenFieldShouldEndEditing:tokenField];
	}
	return YES;
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
	...
	return;//нам нужно оставить отображение токенов после выхода из поля
...
- (void)textFieldDidChange:(UITextField *)textField {
	...
	if ([delegate respondsToSelector:@selector(tokenFieldTextDidChange:textFieldText:)]){
		[delegate tokenFieldTextDidChange:tokenField textFieldText:textField.text];
	}
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
	...
	if ([string isEqualToString:@""] && [textField.text isEqualToString:kTextEmpty] && tokenField.tokensArray.count){
		...
		[tokenField updateHeight:NO];
		if (delegate && [delegate respondsToSelector:@selector(tokenFieldShouldEndEditing:)]) {
            [delegate tokenFieldShouldEndEditing:tokenField];//закрыть список
        }
		return NO;
	}
...
//		[self setBackgroundColor:[UIColor whiteColor]];
        ...
        self.titleColor = newTitleColor;
        self.titleFont = newTitleFont;
		...
		[self setPromptText:newTitleText];
...
- (void)addToken:(NSString *)title {
    [self addToken:title tokenId:nil];
}
- (void)addToken:(NSString *)title tokenId:(NSString *)tokenId{	
    if (![tokenId length]) {
        [self setText:kTextEmpty];
        return;
    }
	...
	if (title){
        if (tokenId != nil) {
            for (TIToken *old in tokensArray) {
                if ([old.tokenId isEqualToString:tokenId]) {
                    return;//прерываем добавление токена, если он уже присутствует в коллекции
                }
            }
        }
		...
		TIToken * token = [[TIToken alloc] initWithTitle:title andId:tokenId];
...
- (void)tokenGotFocus:(TIToken *)token {
    ...
    if (parentView.delegate && [parentView.delegate respondsToSelector:@selector(tokenFieldShouldEndEditing:)]) {
        [parentView.delegate tokenFieldShouldEndEditing:self];//закрыть список
    }
}
...
- (void)scrollTokensUp{
	TITokenFieldView * parentView = (TITokenFieldView *)self.superview;
	CGFloat offset = (numberOfLines == 1 ? 0 : (self.frame.size.height - kTokenFieldHeight) + 1);
	[parentView setContentOffset:CGPointMake(0, self.frame.origin.y + offset) animated:YES];
}
...
- (void)setPromptText:(NSString *)aText {
	...
	CGSize titleSize = [aText sizeWithFont:self.titleFont];
	UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(18.0f, 5.0f, titleSize.width , titleSize.height)];
	[label setTag:123];
	[label setText:aText];
...
#define kTokenCroppedSize 60//было 24 и без константы
...
- (id)initWithTitle:(NSString *)aTitle andId:(NSString *)aTokenId{
	
	if ((self = [super init])){
		
		[self setTitle:aTitle];
        [self setTokenId:aTokenId];
		[self setCroppedTitle:aTitle];
		
		if ([aTitle length] > kTokenCroppedSize){
			NSString * shortTitle = [aTitle substringWithRange:NSMakeRange(0, kTokenCroppedSize)];
...
- (id)initWithTitle:(NSString *)aTitle {
    return [self initWithTitle:aTitle andId:nil];
}
